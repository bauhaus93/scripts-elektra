...
parser.add_argument
parser.add_argument("--target", metavar="target_str", type = str, help = "Set build target", choices = ["all", "default", "lcdproc", "toml", "toml_min"], default = "default")
...
parser.add_argument("--no-install", action = "store_true", help = "Check if don't want to install elektra in container")
...
if __name__ == "__main__":
    ...
    if not (os.path.isdir(ELEKTRA_DIR) and os.path.exists(ELEKTRA_DIR)):
        ...
        exit(1)
    cmd = generate_build_command(CONTAINER_ROOT, args.target, not args.no_install, args.build_type, args.run_tests,  args.run_shell, args.clean_build)
